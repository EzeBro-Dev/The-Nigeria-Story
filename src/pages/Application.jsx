...
// --- THE SMART CHECKER ---
  useEffect(() => {
    const checkExistingApplication = async () => {
      ...
      if (profile?.is_admin) {
        navigate('/dashboard/admin'); // Escort admins directly to the Command Center
        return; 
      }
      ...
      if (app) {
        if (app.status === 'approved') {
          navigate('/dashboard'); // Approved users go to dashboard
        } else {
          setStep(4); // Pending/Rejected users stay in the waiting room
        }
      }
      setIsChecking(false);
    };
        ...
        <div className="p-8 md:p-12">
          ...
          {step === 1 && (
            <div className="animate-fade-in">
              <h2 className="text-3xl font-bold mb-2">Welcome to Cohort 1 👋</h2>
              <p className="text-zinc-400 mb-8">Let's get to know the artist behind the screen.</p>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-bold text-zinc-300 uppercase tracking-wider">Age</label>
                  <input 
                    type="number" 
                    name="age"
                    value={formData.age}
          ...
          {step === 2 && (
            <div className="animate-fade-in">
              <h2 className="text-3xl font-bold mb-2">Your Creative Spark ✨</h2>
              <p className="text-zinc-400 mb-8">What drives you to be part of this historic project?</p>
              
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold text-zinc-300 uppercase tracking-wider">What is your core passion?</label>
                <textarea 
                  name="passion"
          ...
          {step === 3 && (
            <div className="animate-fade-in">
              <h2 className="text-3xl font-bold mb-2">Scholarship Details 🎓</h2>
              <p className="text-zinc-400 mb-8">We are committed to empowering raw talent.</p>
              
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div 
                    onClick={() => setFormData({ ...formData, scholarship_type: 'partial' })}
                  ...
                  >
                    <h3 className="font-bold text-[#FFD700] mb-1">Partial Scholarship</h3>
                    <p className="text-xs text-zinc-400">I can cover some costs, but need a boost.</p>
                  </div>
                  <div 
                    onClick={() => setFormData({ ...formData, scholarship_type: 'full' })}
                  ...
                  >
                    <h3 className="font-bold text-[#FFD700] mb-1">Full Scholarship</h3>
                    <p className="text-xs text-zinc-400">I require complete financial assistance.</p>
                  </div>
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-sm font-bold text-zinc-300 uppercase tracking-wider">Why should we choose you?</label>
                  <textarea 
          ...
          {step === 4 && (
            <div className="text-center animate-fade-in py-10">
              <div className="w-24 h-24 bg-[#FFD700]/10 text-[#FFD700] rounded-full flex items-center justify-center text-4xl mx-auto mb-6 border border-[#FFD700]/30 shadow-[0_0_30px_rgba(255,215,0,0.2)]">
                ⏳
              </div>
              ...
              <p className="text-zinc-400 max-w-sm mx-auto mb-8 leading-relaxed">
                Thank you for applying to Cohort 1! Our team is currently reviewing your application. You will receive an email once decisions are made.
              </p>
              <button 
                onClick={async () => {
